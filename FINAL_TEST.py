...
def main(argv):
    ...
    for i in range(len(paths_rolo)- num_steps):
        ...
        frame = utils.insight_3_locations(img, gt_location, yolo_location, rolo_location)
        video.write(frame)

        utils.createFolder(os.path.join('output/nframes/',sequence_name))
        frame_name= os.path.join('output/nframes/',sequence_name,str(test_id)+'.jpg')
        print(frame_name)
